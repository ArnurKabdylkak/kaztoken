...
int main(void) {
  ...
  const char *names[4] = {"DS1 (P2.2)", "DS2 (P2.3)", "DS3 (P2.4)",
                          "DS5 (P2.5)"};

  while (1) {
    for (int i = 0; i < 4096; i++) {
      if (i == 2000) {
        /* Защита: несколько проверок подряд — glitch должен их "пробить" */
        volatile int safe = 1;
        if (i != 2000)
          safe = 0; /* redundant check — glitch target */
        if (i != 2000)
          safe = 0;
        if (i != 2000)
          safe = 0;
        ...
        if (safe == 0) {
          /* Glitch случился — "невозможная" ветка */
          FIO2CLR = LED_ALL; /* все LED ON */
          puts_("GLITCH! ALL LEDs ON\n");
          while (1)
            ;
        }

        /* Нормальный путь — продолжаем как обычно */
      }
